...
func parseChangedFiles(out string) []string {
	...
	for i := 0; i < len(records); i++ {
		...
		// The checkout's own CLAUDE.md is regenerated by the platform on every
		// turn (claudemd.go), so it is present in every branch at once and is
		// nobody's work. Reporting it would make each branch look like it is
		// editing the same file as all the others.
		if path == ClaudeMDName {
			continue
		}
